...
class EventsController extends Controller
{
    ...
    public function index()
    {
        ...
        $tqtwCheckers = $this->_model->getCheckerEventsForQuestionsWords(Session::get("memberID"));
                        ...
                        if (isset($_POST) && !empty($_POST))
                        {
                            ...
                            if (isset($_POST["confirm_step"]))
                            {
                                ...
                                $chunks = array_filter($chunks, function($v, $key) use ($text) {
                                    ...
                                    if(trim($text[$section]) == "") return true; // do not validate empty source
                                    return !empty($v);
                                ...
                                if(!isset($error))
                                {
                                    if(!empty($translation))
                                    {
                                        foreach ($translation as $key => $chunk)
                                        {
                                            ...
                                            $section = $key+1;
                                            if($section != OdbSections::DATE && trim($text[$section]) != "")
                                                $translation[$key][EventMembers::TRANSLATOR]["verses"] = [($key+1) => $chunks[$key]];
    ...
    public function moveStepBackAlt()
    {
            ...
            $chapter !== null)
        {
            ...
            if(!empty($event))
            {
                ...
                elseif (in_array($event[0]->bookProject, ["tn","tq","tw"]))
                {
                    if(EventStates::enum($event[0]->state) == EventStates::enum(EventStates::TRANSLATED)
